...
class TestYFinanceAdapter:
    ...
    def test_yfinance_adapter_returns_ohlcv(self):
        from datetime import datetime, timedelta
        adapter = YFinanceAdapter(symbol="AAPL", interval="1m")
        # Use recent date (yfinance 1m requires data within last 30 days)
        recent_date = datetime.now() - timedelta(days=2)
        adapter.set_start(recent_date)
        result = adapter.tick()
        assert isinstance(result, list)
        assert len(result) == 5  # OHLCV
    ...
    def test_yfinance_adapter_streams(self):
        ...
        first = adapter.tick()
        second = adapter.tick()
        assert first != second or first is None
